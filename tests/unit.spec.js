...
let entriesToNumbers = app.__get__('entriesToNumbers')
...
describe("#nonNumericEntries", function () {
    ...
    test("when some values are NaN it returns an array of keys with invalid values", function () {
        entries = { a: "4", b: "not a numbeer", c: "also not a number", d: "" }
        expect(nonNumericEntries(entries)).toEqual(['b', 'c', 'd'])
    })
})

describe("#entriesToNumbers", function () {
    test("turns all the values in the entries to numberes", function () {
        entries = { a: 4, b: 5, c: 7 }
        expect(entriesToNumbers(entries)).toEqual({ a: 4, b: 5, c: 7 })
    })
})
